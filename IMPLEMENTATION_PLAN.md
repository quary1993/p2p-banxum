...
- Phase 12 reporting/export foundation is implemented as backend/API only. Admins can generate audited report artifacts through `/api/v1/reporting/admin/reports/` in CSV, deterministic PDF, or ZIP evidence-package format. The request supports custom ranges plus daily, weekly, monthly, quarterly, yearly, and calendar-year presets; `redacted` or `full` mode; optional filters such as currency, participant, operation type, status, pair, Bexio mapping, and destination/handling note. Each generation creates immutable `ReportRun` and `ReportEvent` evidence with DB append-only triggers, report definition version, row count, manifest, file list, content encoding, content checksum, audit event, and domain event. CSV cells are neutralized against spreadsheet formula injection before output. The launch report definitions now include `operational_subledger`, `trial_balance`, `garanta_accrued_revenue`, `bexio_accounting_export`, `bank_operations`, `reconciliation`, `investor_balances`, `balance_ageing`, `withdrawals`, `loan_funding`, `repayment_status`, `default_exposure`, `recovery_write_off`, `fx_activity`, `kyc_status`, `audit_log`, `failed_outbox`, `participant_account_statement`, and `annual_tax_information`. Bexio output is a mapping-driven transaction-line export with unmapped-row markers until the accountant-approved chart/tax/import layout is provided. Participant statements and annual tax-information summaries are ledger/source-model derived for lenders, borrowers, and Garanta internal finance, include information-only principal/balance sections, and carry the informational-only/not-tax-advice disclaimer. The API still returns artifact content in JSON for agent-testability: CSV is plain text; PDF and ZIP are base64. Stored report artifact files/object-storage persistence, large-export streaming/range limits/background generation, polished WeasyPrint layouts, final tax/Bexio wording/mappings, report download UI, and admin-console report screens remain deferred to later reporting/admin-console/technical-hardening slices. Full exports intentionally remain admin-accessible under launch governance, default to redacted mode, and are audited; enhanced approval/reason-code policy is tracked as a Garanta reporting-governance follow-up.
...
- Implement report request system:
  ...
  - implemented: ZIP evidence packages containing CSV, PDF, and manifest.
