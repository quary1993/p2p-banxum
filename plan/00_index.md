...
- Balance source entries older than 30 days become withdraw-only and cannot be newly invested/reinvested. Balance-funded primary-market orders are allowed when consumed source entries are pledged/allocated inside their 30-day investment window, even if the loan funding deadline is later than the source entry's day-30 deadline. Day-60 balances trigger forced withdrawal if a usable IBAN is known or penalty/freeze mode if it is not.
