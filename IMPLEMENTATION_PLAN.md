...
- Phase 8 servicing repayment foundation is implemented as backend/API only for regular and partial next-installment borrower repayments. Admins can record a borrower repayment bank operation, default-match it to the next unpaid installment, require warning acknowledgement for underpayments, reject overpayments until the early/multi-installment slice, apply the launch waterfall with fees/penalties inactive, allocate interest then principal, credit investor balances with new ageing balance lots, and reduce active holding principal using exact `current_principal_minor` weights with deterministic residue handling. Borrower repayment events and lender distribution lines are immutable, have application-level and DB-level append-only guards, and are exposed through `/api/v1/servicing/admin/borrower-repayments/`. Lender-payment-fee fields are reserved but operationally zero at launch; enabling non-zero lender payment fees requires a later ledger path that credits Garanta revenue instead of only investor liabilities. Lender notification emails, admin evidence-file uploads, late/default scheduler, multi-installment payments, early repayment schedule recalculation, full loan-paid status, and recovery waterfall handling remain for later servicing/risk/communications slices.
- Phase 8 servicing status scanning is implemented as backend/API only. Admins can run `/api/v1/servicing/admin/status-scan/` for all active funded/late loans or a selected loan list; the scanner uses Europe/Zurich calendar-day due-date deltas, marks loans late on day 5 and defaulted on day 16 based on the first unpaid installment, records audit/domain/loan events for every status transition, and leaves already-defaulted loans to the recovery workflow. Normal borrower repayment is allowed for funded and late loans, blocked for defaulted loans, and a catch-up repayment that clears the overdue installment automatically moves a late loan back to funded/current. Scheduled daily execution, admin task creation from status changes, investor/borrower notifications, default recovery processing, and full loan-paid status remain deferred.
