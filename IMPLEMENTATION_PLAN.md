...
- Phase 11 risk-note and write-off foundation is implemented as backend/API only. Admins can create immutable internal or public loan risk notes through `/api/v1/servicing/admin/risk-notes/`; public notes are limited to active portfolio loan statuses and cannot use internal/document note types. Admins can list all notes or only public notes. Investors with financial access can read public notes for loans where they have or previously had a holding through `/api/v1/servicing/loan-risk-notes/`, with internal evidence references, metadata, idempotency keys, and admin IDs redacted. Historical note access is intentional so a transferred or closed former holder can still retrieve material public updates for a loan they were exposed to. Admins can record a write-off for an already-defaulted loan through `/api/v1/servicing/admin/write-offs/`; the record stores principal, contractual interest, default interest, fees, penalties, total, reason, notes, evidence reference, prior status, new status, currency, audit/domain events, and a loan event, then moves the loan to terminal `written_off` status. Risk notes and write-off records are append-only at application and DB-trigger level and idempotency-fingerprinted. This slice does not post ledger write-off entries, close or loss-adjust investor holdings, allocate recovered cash, generate recovery/write-off PDFs, or send investor emails; those remain deferred to the recovery distribution, loss-recognition, reporting, documents, and communications slices. Until loss-recognition closes or adjusts holdings, any investor portfolio/holding API or UI must expose the current loan status, including `written_off`, next to the holding so affected investors are not shown the position as a normal live loan.
