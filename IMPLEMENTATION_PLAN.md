...
- Phase 11 risk-note and write-off foundation is implemented as backend/API only. Admins can create immutable internal or public loan risk notes through `/api/v1/servicing/admin/risk-notes/`; public notes are limited to active portfolio loan statuses and cannot use internal/document note types. Admins can list all notes or only public notes. Investors with financial access can read public notes for loans where they have a holding through `/api/v1/servicing/loan-risk-notes/`, with internal evidence references, metadata, idempotency keys, and admin IDs redacted. Admins can record a write-off for an already-defaulted loan through `/api/v1/servicing/admin/write-offs/`; the record stores principal, contractual interest, default interest, fees, penalties, total, reason, notes, evidence reference, prior status, new status, currency, audit/domain events, and a loan event, then moves the loan to terminal `written_off` status. Risk notes and write-off records are append-only at application and DB-trigger level and idempotency-fingerprinted. This slice does not post ledger write-off entries, allocate recovered cash, generate recovery/write-off PDFs, or send investor emails; those remain deferred to the recovery distribution, reporting, documents, and communications slices.
