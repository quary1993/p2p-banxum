...
- With KYB/AML not approved OR a compliance hold: publication, disbursement, and repayment processing are blocked. Current-policy exception (superseding the original 2026-07-03 matrix): routine KYB expiry after publication does not block deterministic funding close, while a hold or adverse/review status does and creates a preserved-reservation operations failure case.
...
21. From publication onward, financeable principal and minimum subscription are immutable to admin edits. Only deterministic deadline close may reduce principal to the subscribed amount when the frozen threshold is met, with an investor notice.
