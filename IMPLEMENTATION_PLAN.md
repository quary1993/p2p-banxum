...
- Phase 5 balance-ageing operations are implemented through backend services, scheduled jobs, investor balances and payout controls, and admin finance and queue screens. Admins can register one active verified payout instruction per investor/currency. Investors can submit or update their own payout IBAN through `/api/v1/ledger/payout-instructions/` after a fresh `bank_account_change` sensitive-action email code; investor-submitted instructions replace the prior active instruction but are stored as `is_verified_usable=false` until Garanta verifies them, so forced withdrawals only use admin-verified payout paths. The admin ageing scan records idempotent reminder-due domain events for days 25, 46, 53, 58, 59, and 60; catches up missed reminder thresholds on the next scan; queues an idempotent `email.balance_ageing_reminder` outbox notice for each newly recorded threshold; writes reminder audit evidence only when the corresponding domain event is newly created; creates forced withdrawal requests for overdue day-60 lots when a verified payout instruction exists; and moves overdue lots into `penalty_mode` with audit/domain evidence when no usable payout instruction exists. The configured `BALANCE_PENALTY_BPS_PER_DAY` and penalty basis are stored in penalty-mode lineage for audit. The platform-core scheduled-job runner can execute the balance-ageing scan daily with durable run evidence. Each scan charges penalty-mode lots at most once per Europe/Zurich business date using the configured simple daily bps, rounded half-up to minor units, capped at the lot's remaining available amount, and idempotent by lot/date. The penalty charge helper locks/refetches the lot and checks same-day charge lineage before posting, so duplicate scans skip without double-mutating the lot even if a journal idempotency race is encountered. The penalty journal debits investor-balance liability and credits Garanta platform-fee revenue; the balance lot moves the charged amount from available to penalized while preserving conservation, and moves to terminal `penalty_exhausted` when the overdue source is fully consumed. Admin task creation from scan results and investor-facing frozen-action enforcement in marketplace/FX flows remain for later Phase 5/security/communications slices. Requested/unfinalized withdrawal cancellation is already implemented and applies to forced withdrawals because forced returns use the same `InvestorWithdrawalRequest` lifecycle.
