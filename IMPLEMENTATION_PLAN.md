...
Canonical naming: BANXUM is the platform/brand name. Garanta Finanzgruppe AG is the legal operator, contracting/regulatory entity, and account-holder identity where applicable. User-facing product copy, portal UI, email templates, generated documents, account statements, tax-information statements, support/FAQ content, and notifications must use BANXUM when referring to the platform brand. Legal footers, operator disclosures, contracts, regulated-entity references, bank/account-holder references, and compliance/regulatory copy must use Garanta Finanzgruppe AG where that legal identity is required. Both values must come from configuration/template variables rather than hardcoded literals in reusable templates or document/report generators.
...
Borrower/originator profiles (2026-09-09): admin-authored stories are structured blocks, never trusted HTML. Direct opportunities expose the borrower story and selected-public fields; LO opportunities expose the originator story, not private final-borrower records. Images require authentication for reading and active-admin permission for upload; accepted raster uploads are capped at 25 MB and 40 million pixels, then re-encoded as metadata-stripped JPEGs at most 1 MB. Story and full schedule/payment pages remain separate with investment entry on both. Previously published direct-loan schedules stay readable after close and include historical repayments/prepayments plus the current remaining schedule. Calculator figures are indicative proportional amounts, not a guaranteed return or replacement for order validation.

Manual regression baseline (2026-09-09): the opt-in staging/local reset profile uses a private mapping for one existing regular admin and three investors. Users 1 and 2 receive CHF/EUR 5,000,000 each, User 3 zero; other investors keep the generic reset default. It can explicitly create missing synthetic verified investors, but does not change existing roles/identity checks or invent document acceptance or payout IBANs. It captures the completed seed with QA enabled: repeated seed restores return to the same initial data and clock, not the state after manual setup. Active regular admins and superadmins can use staging/local QA controls. Production exposes neither navigation nor usable QA endpoints, regardless of role. Separate seed and manual-snapshot restore buttons use private bookmarks that survive either restore; automatic restore defaults to the seed until a manual checkpoint exists. All old sessions are invalidated, but the calling admin receives a fresh session if restored credentials and permissions still match. The guide starts at the exact seed, excludes registration and superadmin-only cases, and assigns clock/restore preparation to the regular Admin. See `QA/Regression-Guide.md` and the deployment runbook.
...
- Phase 2 KYC compliance foundation: user KYC case/session/event records, mock and real Didit hosted-session creation paths, Didit V3 signed webhook processing, provider-status normalization, KYC financial-access gate, audit/domain events, authenticated status/session API, and focused tests.
...
- Numeric balance penalty posting is implemented in the ledger/balance-ageing module. `BALANCE_PENALTY_BPS_PER_DAY` is the operational daily bps source; `balance.penalty_policy_display` remains display copy and must stay consistent with deployed numeric policy.
...
- Real Twilio Verify phone verification is implemented behind `PHONE_VERIFICATION_PROVIDER=twilio_verify`. Local/default `mock` mode still creates a locally hashed/encrypted OTP and queues a redacted SMS outbox message for developer tests. Twilio mode creates the local auditable challenge without storing a local OTP, calls Twilio Verify to send the SMS, stores the Twilio verification SID in `provider_reference`, and confirms codes through Twilio while BANXUM still enforces local challenge ownership, expiry, attempts, throttles, audit events, and phone-verified state transitions. Non-local deploy checks require `PHONE_VERIFICATION_PROVIDER=twilio_verify`, a positive timeout, a Twilio Verify service SID, and either account SID/auth token credentials or API key SID/secret credentials. Twilio API key authentication was validated against the provider on 2026-06-08; SMS cannot be tested end-to-end until a Verify service exists and `TWILIO_VERIFY_SERVICE_SID` is configured. Live Twilio delivery and any Twilio delivery-webhook/suppression behavior still require staging/production provider-side testing.
- Real Didit hosted-session creation is implemented behind `DIDIT_SESSION_PROVIDER=api`, using `x-api-key`, a configured workflow ID, hosted verification URL, contact metadata, and safe non-secret provider payload storage. The local/default provider remains `mock` so development and tests do not create real verification sessions. The Didit API key, workflow ID, and webhook signing secret in the local ignored secret file were validated on 2026-06-05: the workflow is published/default/KYC type, BANXUM successfully created a real hosted Didit KYC session, and the local V3 signature verifier accepts the configured signing secret. End-to-end webhook delivery remains untested until a reachable staging/production domain and live backend are available. Report download, provider artifact storage, and workflow-specific status/report mapping remain deferred until webhook delivery and report/export details are available.
- Didit-specific items that cannot be implemented or fully tested yet:
  ...
  - Didit webhook destination secret and console delivery testing for Garanta's staging/production destination.
  - exact workflow-specific event/status vocabulary beyond the official Didit V3 status envelope and generic AML flag extraction now implemented.
...
- Re-submitting natural-person registration with an existing email recovers only an incomplete self-service account: the existing user must be an active natural-person lender, still `pending_kyc`, not phone-verified, and without a started KYC case. In that narrow state the service refreshes name, phone, marketing consent, records fresh terms acceptance evidence, supersedes any pending phone challenge, and returns the same user id so the investor can restart the phone-verification step. Existing phone-verified, KYC-started, active, restricted, locked, closed, legal-entity, admin, or superadmin accounts still return duplicate-email conflict.
- Didit webhook signature enforcement was hardened. Non-local environments require a valid signature regardless of copied environment overrides, deploy checks flag missing/disabled Didit signature config, and env examples no longer pin an unsafe false value. The webhook endpoint now supports Didit's V3 `X-Signature-V2` canonical JSON signature with `X-Timestamp` freshness, the raw `X-Signature` fallback, and the envelope-only `X-Signature-Simple` fallback.
...
- Didit webhook `raw_payload` currently stores provider evidence as JSON in PostgreSQL. Disk encryption and restricted database access protect the current private-testing environment, but field-level encryption or restricted evidence-object storage is a blocking go-live control before production KYC data is retained.
...
- Sensitive-action email-code verification for investor money actions is intentionally performed after idempotent replay checks but before entering the rollbackable financial transaction. This makes wrong-code attempt accounting and the max-attempt brute-force limit durable even if the later money mutation fails or rolls back. The accepted tradeoff is that a correct code can be consumed by a request that subsequently fails a balance, capacity, quote-expiry, or other business rule; the investor must then request a new code rather than reusing the old one. Future sensitive money actions must follow this placement rule instead of verifying inside the same transaction that mutates balances, holdings, orders, FX, or secondary-market settlement state.
- Investor self-service money actions now have a public authenticated API path to request an action-scoped sensitive email code. The endpoint may issue only investor money-action codes (`withdrawal`, `bank_account_change`, `fx`, `primary_investment`, `secondary_market_listing`, `secondary_market_purchase`); admin-login codes remain admin-auth only.
- Functional v1 provider decisions are: Twilio Email API for transactional email, Twilio Verify for phone verification, Didit for natural-person KYC/KYB/AML provider workflow, and Yahoo Finance for CHF/EUR and EUR/CHF FX rate sourcing and same-provider sanity checks. The legacy SendGrid Web API adapter remains available for rollback, but staging and production use `COMMUNICATIONS_EMAIL_PROVIDER=twilio_email` with a dedicated Twilio Email API-key SID/secret and an authenticated sender domain. The FX quote API now uses a configured provider selector: local/test may use `mock`, while non-local deploy checks require `FX_RATE_PROVIDER=yahoo_finance`. The Yahoo adapter uses the chart endpoint symbol mapping from `fx.yahoo_symbols`, reads `regularMarketPrice`, `regularMarketTime`, and `previousClose`/`chartPreviousClose`, stores only a payload hash reference, and fails quote issuance if Yahoo data is missing, malformed, stale, outside configured bounds, or too far from the same-provider previous-day value. The real-provider integration audit is closed for the implemented code paths: Yahoo/Twilio Email/Twilio Verify fail closed on missing provider configuration and non-local deploy checks prevent accidental mock launch. Weekend and configured holiday closures remain fail-closed, but users now receive explicit messages explaining that live FX market rates are not published; non-calendar provider staleness/malformed provider failures are shown as temporary FX-rate availability issues with support escalation copy. The FX market holiday list is platform-configurable via `fx.market_holidays` using recurring `MM-DD` or exact `YYYY-MM-DD` values. Didit end-to-end webhook/report-artifact testing is deferred until a reachable staging/production domain and live Didit console delivery are available. The FX adapter remains provider-swappable if Yahoo access method, terms, timestamp semantics, or reliability fail pre-launch validation.
- Garanta will provide final legal/business templates. PDF and CSV outputs, where applicable, are required for launch. The current deterministic backend PDF/CSV/ZIP/report foundation is sufficient for source correctness and testing, but production legal/accounting/tax layout/content must be loaded through the document/reporting template workflows before real use.
- Day-60 idle balance handling is force-withdrawal first. If a usable verified payout IBAN exists, the platform attempts forced withdrawal. If forced withdrawal cannot succeed because no usable payout path exists or operational bank return cannot be executed after review, the balance moves into frozen/penalty handling with the configured capped simple daily penalty.
- Final default loss recognition is an advisor-approved resolution workflow, not a normal loan edit. Loans remain `defaulted` while Garanta handles recovery. If recovery is exhausted and Garanta/legal/accounting approve final recognition, the backend can record one strict loss-recognition event for the defaulted loan, close remaining active holdings, and move the loan to `written_off`. This must not be exposed as a casual "remove/write-off" button in the admin console; any launch UI must present the policy checklist, investor impact, evidence reference, and explicit confirmation.
...
- KYC manual review and account access controls are implemented in the backend and admin console. Provider outcomes that need Garanta attention open internal manual review; admins can approve review-routed cases, decline, request re-verification, or reopen; sanctions hits, provider-declined/confirmed-fraud outcomes, never-started cases, pending provider cases, expired cases, and re-verification-required cases cannot be manually approved in the current model. Admins can restrict, lock, close, or reactivate accounts with append-only evidence. Account closure requires explicit admin confirmation and a server-side clean-account check covering available cash, active holdings, open primary orders, requested withdrawals, open secondary listings, unresolved compliance review, and open user-linked admin tasks. `restricted` and `locked` both block account access in the current backend; a later UX/policy pass may distinguish restricted read-only access from full lockout if Garanta wants that distinction. Reversible closure-time privacy pseudonymization remains a blocking policy/key-custody implementation item before production closure requests are processed.
- Phase 3 admin-ops task queues are implemented end to end. Admins can create, list, filter, update, and inspect operational tasks with type, priority, assignment, due date/SLA, related object, notes, status, and completion fields. Task lifecycle evidence is append-only at the application and database levels. The admin console exposes the daily dashboard, task queue, queue drill-downs, reconciliation-break task sync, and audited audit-event search.
- Finance task/action integration is implemented. An investor request to add an unverified payout IBAN automatically creates or reopens a finance task linked to that exact payout instruction; admin verification resolves the linked task. Withdrawal and forced-withdrawal dashboard drawers embed the same finalize/cancel workflow as Finance Operations. Finance Operations begins with a bounded pending-work table that combines unresolved finance tasks with actionable bank-operation, withdrawal, FX-settlement, and reconciliation queues.
...
- Phase 4 borrower-entity foundation is implemented as backend/API plus admin-console UI. Admins can create, list, filter, update, and inspect legal-entity borrower records; set off-platform KYB status and compliance hold; store optional financial disclosure amounts as integer minor units plus currency; manage selected-public borrower fields for business classification, registered address, and contact info; keep ownership, bank-account details, KYB/AML observations, and financial-risk notes internal; link existing stored files as borrower documents; preview investor-facing borrower disclosure with empty optional fields omitted; and inspect append-only borrower events. Authenticated marketplace loan detail reuses the backend `borrower_investor_disclosure` projection so public and selected-public borrower facts are visible to lenders without leaking internal fields. Borrower document upload storage remains handled by the shared stored-file foundation.
- Phase 4 loan product and schedule foundation is implemented in the backend and admin console. Admins can create complete operational loan records, generate monthly repayment schedules for all launch repayment types, use manual schedule overrides at creation/update when principal totals reconcile, inspect LTV warnings, update draft loan data, publish loans only when offline company-compliance safeguards pass (no explicit adverse status or hold), and inspect append-only loan events. Superseded by the 2026-08-05 funding-threshold policy: financeable principal and minimum subscription are frozen at publication; only deterministic deadline close may reduce principal to the amount subscribed when the disclosed minimum is met. Term sanity bounds are implemented as 1 to 600 months. Superseded in the refinancing-loans slice: the first-installment-date input was removed everywhere; the loan start date is declared when the loan is defined (defaulting to the funding deadline) and the first installment is always due one month after the loan start date. Exact golden schedule fixtures pin every launch repayment-type mapping, and the Manage dialog shows the generated schedule before publication.
- Phase 5 ledger foundation is implemented end to end through investor and admin surfaces. Admins can declare reconciled lender deposits, which create immutable bank-operation evidence, balanced double-entry journal entries/postings, and investor balance lots with Europe/Zurich 60-day holding deadlines with loan-specific funding eligibility derived from bank value date. The ledger exposes admin balance summaries, FIFO pledge-eligibility planning for balance-funded investment, investor withdrawal requests, admin withdrawal cancellation before bank payout, admin withdrawal finalization, borrower disbursement finalization, FX source-balance conversion/target-balance crediting, FX external settlement cash clearing, balance-ageing penalty charging, and reconciliation snapshots comparing bank-stated balances to investor-balance liabilities plus withdrawal, borrower-disbursement, FX-clearing, and Garanta revenue balances. Investor-initiated withdrawal requests now require a fresh `withdrawal` sensitive-action email code consumed through the shared platform-core facade before any balance-lot mutation; idempotent replays return existing requests without consuming another code. Withdrawal requests consume FIFO balance lots and post an internal immutable journal from investor balance liability to withdrawal payable; admin cancellation of a requested/unfinalized withdrawal restores the exact lot allocations and posts a reversing withdrawal-payable-to-investor-liability journal; admin finalization creates a lender-withdrawal bank operation and posts withdrawal payable to collection cash. Primary-market close moves loan-funding escrow into borrower-disbursement payable for the full accepted principal, without treating lender principal as Garanta revenue; superseded in the refinancing-loans slice, no fee is accrued at close and the planned fee is recorded as evidence only. Admin borrower-disbursement finalization creates a borrower-loan-disbursement bank operation with settlement evidence references, posts borrower-disbursement payable to collection cash, and books the borrower success fee as Garanta revenue at disbursement; the default payout is the funded principal minus the BANXUM fee (default fee = funded principal x `borrower_success_fee_bps`, default 2%), the default amount and fee are readonly and can only be overwritten together with a required explanation note, validations require borrower transfer plus fee to equal and fully clear the outstanding payable, with the fee between 0 and 10% of the funded amount; no residual borrower payable is permitted, and the flow locks the funded loan row before checking payable/cash balances, validates the request borrower against the loan borrower, re-checks borrower KYB/no-hold eligibility at payout time, and moves the loan from `funded` (funding closed, awaiting payout) to `active` only after the disbursement journal and bank operation succeed. FX conversion debits source-currency investor liability and credits source-currency FX clearing; credits target-currency investor liability from target-currency FX clearing; posts FX fee revenue separately; consumes source balance lots FIFO; records converted source-lot amounts in the lot conservation invariant; creates a target FX-proceeds balance lot; and inherits the earliest consumed source-lot investment/withdrawal deadlines without resetting ageing. FX external settlement posts the actual source-currency cash out and target-currency cash in against FX clearing, leaving any residual surplus/deficit visible in FX clearing and reconciliation without retroactively changing investor balances. Bank operations, journal entries, postings, and reconciliation snapshots have application-level and DB-level append-only guards, and the raw SQL immutability test now asserts the append-only trigger message instead of relying on SQLite parameter errors. Balance lots have DB/service conservation checks, terminal-status zero-available checks, and are read-only in Django admin. Ledger idempotency keys now store request fingerprints and reject replay with different payloads; derived journal keys are bounded to the database key length. Reconciliation snapshots now include current-state semantics metadata, investor liability posting totals, signed credit-balance components for withdrawal payable/borrower-disbursement payable/Garanta accrued revenue/FX fee revenue/FX clearing/suspense, account-sign anomaly metadata, collection-cash ledger comparison, and investor-level lot-versus-posting integrity break metadata/events. Reconciliation-break snapshots can be synchronized into idempotent admin tasks and surface in the operations dashboard. Balance-lot mutation beyond deposit/withdrawal/primary reservation/release/FX/servicing crediting/penalty charging, forced-withdrawal task generation, and uploaded bank-statement/evidence-file attachment to borrower disbursements remain for later Phase 5+ slices.
- Lender-deposit source-bank control is implemented. Every admin deposit declaration requires the payer/source IBAN from the bank statement. The service normalizes it and validates its country-specific length and MOD-97 checksum before any bank operation or ledger mutation. A valid deposit atomically establishes that exact source IBAN as an active, admin-verified payout instruction for the investor and currency; replay returns the same deposit result and does not duplicate the instruction.
- Phase 5 balance-ageing operations are implemented through backend services, scheduled jobs, investor balances and payout controls, and admin finance and queue screens. Distinct IBANs are additive, so an investor can retain multiple active payout instructions in one currency. The deposit source IBAN is verified automatically. An investor can request another payout IBAN through `/api/v1/ledger/payout-instructions/` after a fresh `bank_account_change` sensitive-action email code; that exact instruction remains unverified and unusable until admin review, creates a linked finance task automatically, and does not disable an existing verified deposit IBAN. Re-submitting the same verified IBAN does not downgrade it. The admin ageing scan records idempotent reminder-due domain events for days 25, 46, 53, 58, 59, and 60; catches up missed reminder thresholds on the next scan; queues an idempotent `email.balance_ageing_reminder` outbox notice for each newly recorded threshold; writes reminder audit evidence only when the corresponding domain event is newly created; creates forced withdrawal requests for overdue day-60 lots when a verified payout instruction exists; and moves overdue lots into `penalty_mode` with audit/domain evidence when no usable payout instruction exists. Each scan charges penalty-mode lots at most once per Europe/Zurich business date using the configured simple daily bps, rounded half-up to minor units, capped at the lot's remaining available amount, and idempotent by lot/date. The penalty charge helper locks/refetches the lot and checks same-day charge lineage before posting, so duplicate scans skip without double-mutating the lot even if a journal idempotency race is encountered. The penalty journal debits investor-balance liability and credits Garanta platform-fee revenue; the balance lot moves the charged amount from available to penalized while preserving conservation, and moves to terminal `penalty_exhausted` when the overdue source is fully consumed. Requested/unfinalized withdrawal cancellation applies to forced withdrawals because forced returns use the same `InvestorWithdrawalRequest` lifecycle.
- Phase 6 document templates, clickwrap, and downloadable evidence are implemented through backend services, investor registration, transaction and Documents flows, and superadmin/admin template and user-document screens. Superadmins can create immutable template versions, publish a version directly, publish a draft by creating an immutable published copy, retrieve the current published template by category/key/language, and record clickwrap acceptance evidence against the server-owned current template hash. Acceptance evidence is append-only, idempotency-fingerprinted, stores template version, content hash, checkbox labels, context, server-owned user/platform/operator data snapshot, IP/user-agent, audit evidence, and domain events. Primary/secondary transaction acceptances are gated by the shared financial-access predicate. Registration uses the canonical registration document template whenever one is published: the frontend fetches the server-published lender user agreement, the registration API requires the current template version and exact checkbox label, and the legacy env-backed `REGISTRATION_TERMS_VERSION`/`REGISTRATION_TERMS_HASH` path remains only as a local/bootstrap fallback when no registration template exists. Registration also requires a separately current server-published risk-disclosure version and records independent immutable risk acceptance evidence in the registration context. The public current-template endpoint exposes only user-facing template fields and omits operational publication metadata. Acceptance payloads have explicit launch caps: up to 20 checkbox labels, up to 500 characters per label, and up to 64 KiB each for data snapshot and metadata. Clients must send the `expected_template_version_id` returned by the current-template endpoint for every acceptance request so stale-template retries fail with a clear version error instead of an idempotency mismatch. Generic PDF and CSV rendering for accepted/generated evidence is implemented: artifacts are regenerated from immutable template version plus acceptance snapshot, missing variables fail closed, CSV formula cells are neutralized, output includes checksum/manifest/renderer version, and append-only `DocumentRenderedArtifact` metadata records the rendered artifact without treating generated bytes as the permanent source of truth. Generated PDFs and CSVs, where applicable, are required for launch; clickwrap evidence plus email confirmation alone is not sufficient as the production document-delivery model. New acceptance evidence no longer queues legal-document PDF emails. Investors see historical accepted-document records in the portal Documents section with human-readable labels, and admins can open the Users document-history modal to generate the same evidence on demand with `admin_download` audit attribution. Legacy queued `email.document_acceptance_pdf` outbox rows render as portal notices with no attachment. The Garanta lender user agreement DOCX can be imported with `import_lender_user_agreement`, which extracts the final agreement body and recommended checkbox text, stores drafting instructions as metadata, replaces the effective-date placeholder, and blocks publication while `[to confirm]`/`[to complete]` placeholders remain unless explicitly overridden for non-production testing. The Garanta project investment confirmation DOCX can be imported with `import_project_investment_confirmation` as the current `primary_market_investment/default/en` template. It maps the source bracket fields into document placeholders, uses the recommended confirmation text as the required clickwrap checkbox, records source hashes/metadata, and renders accepted-document PDFs/CSVs on demand from the same acceptance/evidence workflow. For private testing only, `seed_demo` publishes temporary `secondary_market_listing` and `secondary_market_purchase` placeholders when no current template exists; the seed is conservative and never replaces a superadmin-published current template, but the placeholder metadata explicitly marks it as non-approved legal content that must be replaced before real-money production secondary-market use. For real primary-order contexts, the document service enriches the acceptance snapshot with server-owned order, loan, borrower, lender, assignment, and holding-placeholder values so investor-supplied snapshots cannot forge transaction terms. Holding IDs are shown as assigned at funding close until the primary close creates holdings; if legal requires the final holding ID in the agreement itself, a later post-close document-regeneration slice must promote that value into a separate final assignment artifact. The superadmin template editor and publication flow are available; final WeasyPrint production layouts, variable preview/test-send tooling, and legal-review attachment workflows remain follow-up items.
...
- Phase 8 servicing status scanning is implemented through backend services, scheduled jobs, and admin servicing controls. Admins can run `/api/v1/servicing/admin/status-scan/` for all `active`/`late` loans or a selected loan list; funding-closed loans remain `funded` and outside servicing until borrower disbursement moves them to `active`. The scanner uses Europe/Zurich calendar-day due-date deltas, marks loans late on day 5 and defaulted on day 16 based on the first unpaid installment, marks loans repaid when no scheduled installment remains outstanding, records audit/domain/loan events for every status transition, and leaves already-defaulted loans to the recovery workflow. Normal borrower repayment is allowed for `active` and `late` loans, blocked for `funded` and `defaulted` loans, and a catch-up repayment that clears the overdue installment automatically moves a late loan back to `active`/current or `repaid` when all installments are cleared. The platform-core scheduled-job runner can execute the status scan daily with `business_date(now_utc())` semantics. Admin task creation from status changes, investor/borrower notifications, and default recovery processing remain deferred. Status-scan batching/per-loan transactions remain a scale hardening item; the current scan is acceptable for launch-scale portfolios.
- Phase 9 FX quote/execution and external settlement are implemented through backend services, investor FX screens, and admin settlement operations. Investors with financial access can request fixed 60-second executable FX quotes for enabled pairs, currently CHF/EUR and EUR/CHF, with no minimum exchange amount, configurable 1.5% default platform fee, half-up minor-unit rounding, at-least-6-decimal stored rate precision, sanity checks for stale/nonpositive/malformed/out-of-bound rates and +/-5% previous-day-average deviation, and a configurable CHF 100,000 equivalent per-investor daily limit. The amount-entry screen uses a non-persistent provider-backed preview endpoint that applies the same access gate, enabled-pair check, provider freshness/sanity controls, fee, exact minor-unit rounding, and daily-limit calculation as executable quote issuance. Previewing never creates a quote, append-only event, audit record, or execution entitlement; pressing Convert always requests a fresh executable quote and the confirmation screen remains bound to that server-issued quote and its expiry. Quote issue and quote execution are idempotency-fingerprinted and append-only. The quote API now uses `FX_RATE_PROVIDER`: local/test mock rates are allowed only in local/test environments; non-local deploy checks require Yahoo Finance. The Yahoo adapter fetches chart payloads for configured `fx.yahoo_symbols`, derives the executable rate and previous-day comparison from Yahoo data, and fails closed if provider data is malformed, stale, missing timestamps, outside configured bounds, or too far from previous-day value. Executing a quote requires a fresh `fx` sensitive-action email code consumed through the shared platform-core facade, then instantly consumes eligible source-currency balance lots, blocks penalty-mode investors from new FX activity, credits a target-currency FX-proceeds lot with inherited source deadlines, posts balanced source and target journals, posts FX fee revenue separately from client-money flows, records audit/domain/FX events, and preserves source-lot lineage. Admins can query an unsettled FX delta report by date range showing source sold by currency, gross target bought by currency, target credited, fees, and net external settlement amounts for exchanges not yet linked to an external settlement. Admins can declare the actual external FX settlement for a pair/date range with sold/bought currencies, actual bank-side sold/bought amounts, booking/value dates, bank references, evidence reference, and notes; the platform stores expected internal sold/bought amounts, inferred actual execution rate, residual surplus/deficit by currency, two linked bank operations, two linked ledger journals, audit/domain events, append-only FX evidence, and append-only per-exchange settlement links. Each FX exchange can be linked to only one external settlement, so overlapping date ranges cannot double-settle already settled exchanges. Admins can query realized settlement reports by value-date range showing expected/actual sold and bought amounts, fees, and residuals by currency. Live Yahoo reliability and terms validation and FX settlement evidence-file upload remain provider and operations hardening items.
- Phase 10 secondary-market listing and purchase/settlement are implemented through backend services, investor marketplace screens, and admin approval operations. Investors with financial access can list one full active holding after current listing-terms evidence and a fresh action-scoped email code. Holdings in disbursed/performing `active` loans publish automatically; `funded` loans are still awaiting borrower payout and are not listable; late/defaulted holdings require admin approval and public disclosure. Seller `price_bps` is the durable premium/discount choice. Borrower repayments, repayments in advance, recoveries, and servicing-status changes atomically recompute every unsold open listing from latest holding/loan data while preserving that `price_bps`; zero/unlistable holdings auto-cancel, active loans remain visible, and late/defaulted listings return to approval-required status. Repricing recalculates principal, transfer price, ACT/365 accrued interest, fees, seller proceeds, buyer cost, last-payment/DPD/risk fields, and avoids re-accruing interest already paid by an early regular installment. Listing transitions are idempotency-fingerprinted and append-only. Buyer-facing projections preserve counterparty redaction. Purchase requires current purchase terms, a fresh code requested only when the buyer chooses to proceed, and risk acknowledgment for non-standard loans; settlement recomputes economics at purchase date, consumes eligible balance, posts balanced liabilities/fee revenue, transfers the full holding atomically, stores immutable purchase evidence, and sends redacted buyer/seller confirmations. Assignment/transfer PDF generation and final legal notification wording remain follow-ups.
- Phase 11 risk-note, recovery-payment, and final loss-recognition workflows are implemented through backend services, investor risk-note visibility, and admin servicing operations. Admins can create immutable internal or public loan risk notes through `/api/v1/servicing/admin/risk-notes/`; public notes are limited to active portfolio loan statuses and cannot use internal/document note types. Admins can list all notes or only public notes. Investors with financial access can read public notes for loans where they have or previously had a holding through `/api/v1/servicing/loan-risk-notes/`, with internal evidence references, metadata, idempotency keys, and admin IDs redacted. Historical note access is intentional so a transferred or closed former holder can still retrieve material public updates for a loan they were exposed to. Admins can record recovery payments for defaulted loans through `/api/v1/servicing/admin/recoveries/`; the event stores gross recovered amount, externally deducted costs, net received amount, third-party costs deducted from received funds, explicit recovery-fee decision/bps/base/amount, net amount available for lender distribution, admin-declared outstanding penalty/default-interest and contractual-interest obligations, server-derived allocation under the fixed universal waterfall, bank/evidence references, audit/domain events, immutable per-investor distribution lines, and idempotent `email.recovery_distribution_credited` notices for affected lenders. The server always applies Garanta legal costs and recovery fee first, then penalty/default interest, then contractual interest, and only then principal derived from current holdings; neither the admin nor a project configuration can change that priority. Recovery distribution uses exact current-principal holding weights with deterministic largest-remainder rounding, credits investor balances as new recovery balance lots with fresh 60-day holding deadlines, reduces recovered principal from current holdings, credits Garanta recovery fees to platform-fee revenue, and keeps recovery-cost amounts deducted from received funds in recovery payable so reconciliation remains balanced. Admins can record final default loss recognition through `/api/v1/servicing/admin/write-offs/` only for already-defaulted loans; the workflow requires written-off principal to equal remaining active holding principal, creates immutable write-off evidence plus per-investor `InvestorLossRecognitionLine` records, allocates interest/fee/penalty loss components by exact holding weights, closes every active holding, and moves the loan to terminal `written_off`. `LoanWriteOffEvent` has both idempotency-key uniqueness and a loan-level unique constraint, so a loan can have at most one final loss-recognition event at the database level. No bank/cash ledger journal is posted for this loss-recognition step because the investor principal already left investor balance liability when the loan funded; loss lines and holding closure are the authoritative investor-exposure evidence for reporting/tax outputs. Risk notes, recovery records, recovery distribution lines, write-off events, and loss-recognition lines are append-only at application and DB-trigger level and idempotency-fingerprinted. This slice does not automatically calculate default/penalty interest from legal agreements, generate recovery/default PDFs, decide when Garanta should trigger final loss recognition, or allocate recoveries received after a loan has already been written off; those remain legal/accounting/operations policy decisions. Recovery payments currently do not transition a loan from `defaulted` to a separate recovered/resolved terminal status, even if holding principal reaches zero; that terminal-status policy is deferred to Garanta product/accounting/legal decision before recovery-closure reporting is implemented.
- Phase 12 reporting/export is implemented through backend services, investor self-service downloads, and admin report-generation and download screens. Admins can generate audited report artifacts through `/api/v1/reporting/admin/reports/` in CSV, deterministic PDF, or ZIP evidence-package format. The request supports custom ranges plus daily, weekly, monthly, quarterly, yearly, and calendar-year presets; `redacted` or `full` mode; optional filters such as currency, participant, operation type, status, pair, Bexio mapping, and destination/handling note. Each generation creates immutable `ReportRun` and `ReportEvent` evidence with DB append-only triggers, report definition version, row count, manifest, file list, content encoding, content checksum, audit event, and domain event. CSV cells are neutralized against spreadsheet formula injection before output. ZIP evidence packages use fixed entry metadata so package checksums are reproducible when the same source data and generation metadata are used. The launch report definitions now include `operational_subledger`, `trial_balance`, `garanta_accrued_revenue`, `bexio_accounting_export`, `bank_operations`, `reconciliation`, `investor_balances`, `balance_ageing`, `withdrawals`, `loan_funding`, `repayment_status`, `default_exposure`, `recovery_write_off`, `fx_activity`, `kyc_status`, `audit_log`, `failed_outbox`, `participant_account_statement`, and `annual_tax_information`. Full/unredacted exports require an active superadmin. Restricted compliance/tax/statement/audit reports (`kyc_status`, `audit_log`, `participant_account_statement`, and `annual_tax_information`) require an active superadmin even when redacted, until a dedicated compliance/finance export role is implemented. Bexio output is a mapping-driven transaction-line export with unmapped-row markers until the accountant-approved chart/tax/import layout is provided. Participant statements and annual tax-information summaries are ledger/source-model derived for lenders, borrowers, and Garanta internal finance, include information-only principal/balance sections, and carry the informational-only/not-tax-advice disclaimer. The API still returns artifact content in JSON for agent-testability: CSV is plain text; PDF and ZIP are base64. Stored report artifact files/object-storage persistence, large-export streaming/range limits/background generation, polished WeasyPrint layouts, final tax/Bexio wording/mappings, and any finer-grained compliance/finance export role remain deferred to later reporting/admin-console/security-hardening slices.
- Phase 13 investor portal is implemented as a typed first-version frontend plus self-scoped backend/API projections. Investors with financial access can retrieve dashboard, balances, portfolio, recent activity, primary orders, secondary-market own listings/purchases/sales, FX quote/exchange history, configured deposit instructions, document/evidence/statement/tax download options, and notification delivery status through `/api/v1/investor/portal/`; no endpoint accepts a client-supplied investor ID. Deposit instructions are sourced from the admin-configurable `payments.deposit_instructions_by_currency` platform setting and include a server-generated investor/currency payment reference that must be used in bank-transfer descriptions for reconciliation. Investor document downloads are self-scoped: accepted clickwrap evidence can be downloaded as PDF/CSV, and account statements plus annual lender tax-information statements are generated on request through a self-service reporting entry point that hard-codes `participant_type=lender` and `participant_id=request.user.id`. Self-service report runs are still recorded as immutable `ReportRun`/`ReportEvent` evidence with `run_mode=investor_self_service`, while admin full/restricted export governance remains unchanged. Notification status is derived from email outbox/delivery evidence for the authenticated investor; magic-link and sensitive-action-code email bodies are never shown in the portal so email remains the out-of-band step-up channel. The integrated Vite/React portal uses generated API hooks, live API by default, explicit non-production fixture preview mode, retryable error/empty/loading states, and responsive screens for public preview, login, registration/KYC handoff, dashboard, marketplace, loan detail, portfolio, balances, withdrawal, deposit instructions, payout IBAN update, FX, secondary market, documents/downloads, notifications, settings, and FAQ. The first live investor write-flow slice now wires magic-link request/consume, natural-person registration submission, KYC session start/status, sensitive email-code request, investor withdrawal request, investor payout IBAN submission/update, FX quote/execute, primary investment order/create-clickwrap-allocate, secondary listing create, and secondary purchase settlement through generated API client hooks. Transactional clickwrap flows fetch the current server-published template, submit `expected_template_version_id`, and accept the server-provided checkbox labels rather than hardcoding legal labels in the client. Investor write actions remain server-enforced by KYC/phone/account financial-access gates where applicable, sensitive-action code checks, idempotency keys, balance/capacity/quote/listing validation, and ledger/holding services. Investor-submitted payout IBANs are not treated as usable until admin verification. Still pending for investor-facing launch readiness: final legal copy/templates, advisor-approved production PDF/CSV layouts and tax wording, provider-backed SendGrid/Twilio/Didit/Yahoo deployment validation, and Claude Design's production visual/accessibility polish pass. Portfolio pagination/bounds, batched days-past-due snapshots, and large-portfolio query optimization remain deferred to the investor-portal scale pass because launch traffic and portfolio sizes are expected to be small.
- Smart Invest is implemented as a self-scoped, server-owned discovery rule with immutable rule events and deduplicated first-publication match evidence. Each eligible investor may activate one current rule from structured Marketplace criteria, replace it by saving new criteria, or deactivate and clear it; admins cannot edit investor rules. Marketplace filters can be saved into the rule, the Smart Invest page and dashboard show current matches, and matching newly published opportunities queue mandatory transactional email/portal alerts once per investor and loan. Search text, sorting, and display mode are not persisted. The five-step wizard contains collateral, CHF/EUR scope, optional minimum yield, optional maximum term, and review only; it deliberately omits repayment/reinvestment and originator-concentration questions. Smart Invest never reserves balance, creates an order, purchases a claim, ranks suitability, or invests automatically.
...
- Primary-market funding cancellation and deterministic deadline resolution are implemented. Admins may deliberately cancel a still-open campaign through `/api/v1/marketplace/primary/admin/loans/{loan_id}/cancel-funding/`. After a funding deadline passes, `/api/v1/marketplace/primary/admin/loans/expiry-scan/` applies the stored per-loan minimum subscription: at/above the threshold it closes at the committed amount; below it, it cancels. Cancellation releases every allocated order through the audited ledger/order release primitive, restores exact balance-lot allocations, posts reversing escrow-to-investor-liability journals, decrements committed principal to zero, closes pending intents as `closed_not_invested`, moves the loan to `cancelled`, and records immutable evidence/events. A processing failure instead moves the campaign to non-public `funding_close_failed`, leaves all reservations untouched, creates/reopens one urgent admin task, and emails `OPERATIONS_ALERT_EMAIL`. The scheduler automatically retries the same threshold-determined outcome; admins repair the cause but cannot override a qualified failed close with cancellation or release its reservations. Failed per-loan resolutions mark the scheduled run failed for monitoring and retry. Deployments must install the frequent retry invocation documented in the deployment runbook.
- Production hardening test readiness is implemented. CI runs the backend suite against PostgreSQL 16, and `MIGRATION_CHECK_DATABASE_URL` is set to PostgreSQL in CI so migration drift is checked against the production database engine rather than SQLite. The repository also exposes `make test-backend-postgres`, `make migration-check-postgres`, and `make test-postgres-hardening`; the focused hardening pack verifies DB-level append-only trigger enforcement through raw SQL and a concurrent same-idempotency-key lender-deposit race against PostgreSQL. Local SQLite checks remain useful for fast development, but first production money movement requires the Postgres hardening pack to pass against a disposable/staging Postgres database. Never run pytest against the production database.
- Scheduled-job monitoring readiness is implemented. The `check_scheduled_jobs` management command fails non-zero when any selected job has unresolved `failed` evidence or a `running` run older than `SCHEDULED_JOBS_RUNNING_TIMEOUT_MINUTES`; `make check-scheduled-jobs` wraps the command for local/operator use. Deployment monitoring should run this check from cron, systemd timers, or the future scheduler/worker infrastructure and alert on non-zero exit. Wiring the runner plus monitor plus production alert route is a blocking operational launch step and is tracked in `admin_todo_tech.md`; the code-side monitor gap is closed. The actual scheduled-job runner still owns idempotency, retry, stale-run reclaim, and append-only run evidence.
- Production-hardening audit disposition: no code issue remains from the readiness slice. Additional PostgreSQL-only concurrency tests for withdrawal finalization and secondary-market double-purchase races are accepted as useful breadth but deferred to a later hardening-depth pass because the current pack already validates DB append-only behavior and the canonical financial idempotency race on PostgreSQL, while those services have row locks, unique constraints, idempotency fingerprints, and focused service tests.
...
- Final counsel/VQF/SRO/bank validation of the user-balance model, loan-specific remaining-funding-window eligibility, 60-day withdrawal requirement, forced withdrawal authority, and day-60 penalty wording.
...
- SES is not required because Twilio Email API is the launch email provider.
...
- Natural-person KYC evidence/manual review and explicit access restrictions; company KYB remains offline with optional platform references.
...
- `borrower_can_transact` is the shared gate for publication, deterministic funding close, disbursement and borrower servicing. It blocks explicit holds and declined/manual-review decisions. Company KYB is completed offline; absent, pending, not-started or expired optional platform records do not block transactions.
...
- Funding close atomically activates holdings/post-boundary rights and moves escrow
  to internal originator payable using the original published schedule. There is no
  separate activation task or required bank receipt. The boundary installment belongs
  wholly to the LO and is recorded later through borrower repayment. Failed close
  preserves reservations and creates an urgent task/email. Historical upgrade exceptions
  require repair or cancellation/refund, never invented payment evidence.
...
- Implement legal-entity lender admin creation:
  ...
  - Optional references to offline company onboarding/KYB records; no local evidence pack is required.
  - Financial access requires an active, unrestricted, phone-verified representative; Garanta completes company KYB offline.
  - One representative login can behave like a normal lender under those access controls.
- Enforce explicit borrower compliance holds and adverse/review decisions across origination, marketplace and payments; missing or expired optional company KYB data is not a gate.
...
- Legal-entity lenders rely on Garanta's offline company KYB and an active, unrestricted, phone-verified representative; no local company approval record is required.
- Borrower transactions assume completed offline company KYB; explicit compliance holds and adverse/review decisions remain blocking.
...
- Implement optional borrower display fields:
  - business classification (admin-selected public per borrower).
  - registered address (admin-selected public per borrower).
  - contact info (admin-selected public per borrower).
  - assets.
  - liabilities.
  - revenue last year.
  - profit last year.
...
- Implement internal-only borrower operational fields:
  - ownership.
  - bank accounts.
  - KYB/AML observations.
  - financial risk.
...
- Enforce funding deadline defaults:
  - draft/admin planning maximum 50 inclusive subscription dates.
  - default 30 inclusive subscription dates from the Europe/Zurich business date.
  - both drafts and publication permit at most 50 inclusive subscription dates. Publication freezes the deadline. Every reserved source must cover the remaining window within its original holding limit; see PAY-DEC-031.
...
- Allow draft loan fields to be edited subject to validation and audit logging.
- Freeze the financeable principal and disclosed minimum subscription at publication. Only the deterministic funding-deadline resolver may reduce principal to the subscribed amount when the frozen threshold is met; it records the required investor message/reason and notification evidence.
...
- Implement balance lots per investor and currency:
  ...
  - Loan-specific investment eligibility within the original 60-day holding deadline.
...
- Implement primary-market balance-source eligibility at pledge/allocation time:
  - consumed source lots must have enough remaining holding time for the full remaining funding window.
  - the last inclusive funding date must precede every consumed source's withdrawal-deadline date.
  - otherwise block the order and show the amount as withdraw-only for new investments.
...
- A source can be older than 30 days and still fund a shorter campaign, provided its remaining holding time covers that campaign.
...
- 60-day holding ageing tests with time travel.
...
- Implement clickwrap acceptance evidence:
  ...
  - user agent.
  - transaction context.
- Implemented: PDF and CSV generation from accepted/generated document snapshots.
- Implemented: secure document access in investor portal for acceptance evidence downloads.
...
- Implement email templates for:
  ...
  - automatic partial funding close with the final subscribed amount and investor notice.
...
- Generated PDFs/CSVs are reproducible from stored template/data snapshots.
...
- Implemented: PDF/CSV snapshot, checksum, access-control, and append-only artifact metadata tests.
...
- Implement investment order creation:
  ...
  - Uses available sources whose remaining holding time covers the selected funding window.
...
- Implement deterministic funding-deadline resolution from a per-loan minimum subscription, default 50% and immutable once the loan is published.
- At/above the threshold, close automatically at the subscribed amount; below it, cancel and restore reservations.
- For an automatic partial close, set the subscribed amount as final financeable principal, regenerate the repayment schedule from that principal with the same term, interest percentage, and percentage BANXUM fee, and require borrower repayment of that principal plus agreed interest. Refinancing original loan data is preserved.
- If resolution fails, unpublish into `funding_close_failed`, preserve reservations, create an urgent admin task, email operations, and retry the same deterministic outcome automatically; admins cannot cancel a qualified round instead.
- Implement investor notifications for partial funding and amount changes.
- Implement borrower success-fee accounting record:
  - Percentage fee, default 2% of funded principal (`borrower_success_fee_bps`), overridable within 0 to 10% together with the disbursement amount and a required explanation note.
  - Deducted from disbursement and booked as Garanta revenue at disbursement; no fee accrual at funding close (planned fee recorded as evidence only).
  - No effect on borrower repayment schedule.
- Implement borrower disbursement workflow and evidence upload:
  - Backend ledger/API finalization is implemented with the default funded-principal-minus-fee amount and a joint amount/fee override with required note. Transfer plus fee must exactly equal and fully clear the funded borrower payable; integer minor-unit accounting permits no residual tolerance. Successful finalization moves the loan from `funded` to servicing-ready `active`.
  - The disbursement action is available from the Loans table Manage dialog for funded loans and from the Finance ops card. The collection-account identifier defaults server-side from the configured currency collector when the client leaves it blank; uploaded bank-statement/evidence-file attachments remain to be built.
...
- Partial funding follows the disclosed per-loan threshold automatically and is notified.
...
- Full and threshold-based partial funding close tests.
- Exact minor-unit threshold, deadline race, cancellation/refund, failed-resolution preservation, task/email alert, and retry tests.
...
- Implement admin borrower payment event entry:
  ...
  - evidence upload.
  - notes.
- Require the fixed outstanding amount of the next due installment for regular installment declarations; the amount cannot be edited.
- If a regular installment is recorded more than one day before its due date, require explicit admin acknowledgment that it is a timely installment collecting the full contractual interest; otherwise direct the admin to repayment in advance.
- Route any other amount (except defaulted loans, which go through recovery) through the "Repayment in advance" declaration with a borrower repayment bank date.
- Apply the repayment-in-advance waterfall:
  - fees.
  - penalties.
  - all unpaid scheduled interest of installments due on or before the bank date.
  - exact ACT/365 accrued interest on outstanding principal from the latest interest-paid-through date up to, but excluding, the bank date; regular installments cover interest through their contractual due date, repayments in advance cover through their actual bank date, and a same-day second advance accrues zero new interest.
  - outstanding principal with the remainder.
- Keep penalties configurable and set to zero/inactive at launch.
- Show a preview/confirmation dialog with the allocation and both old and new schedules before writing (`POST /api/v1/servicing/admin/borrower-repayments/advance-preview/`).
- Implement partial, multi-installment, and full early repayments through the repayment-in-advance flow.
- Regenerate the future schedule as a new version after a repayment in advance: only future rows, original due dates preserved, first regenerated installment interest calculated ACT/365 from the bank date to its due date; mark the loan repaid on full payoff. Investor/admin full-loan views merge immutable historical repayment events with the latest outstanding schedule, while holding projections remain future-only.
- Reprice every open, unsold secondary-market listing after a repayment, recovery, or servicing-status change from the latest holding/loan state while preserving only the seller's `price_bps` premium/discount selection. Notify a repayment recipient about automatic repricing only while their listing remains active.
...
- Regular fixed-amount installment golden test.
- Repayment-in-advance partial-amount golden test.
- Repayment-in-advance multi-installment late-loan golden test.
- Repayment-in-advance interest waterfall and proration golden test.
- Repayment-in-advance full-payoff test.
...
- The configured mock FX provider is local/test-only and fails closed outside local/test environments; staging and production deploy checks require Yahoo Finance provider configuration.
...
- Implemented: Yahoo Finance adapter parsing tests and local live-smoke verification against the Yahoo chart endpoint.
- Remaining: Yahoo Finance legal terms/rate-limit review, weekend/holiday freshness policy, and display-rate polling tests.
...
- Implemented: require listing terms checkbox and fresh sensitive-action email code.
...
- Implement buyer purchase:
  ...
  - Implemented: requires fresh sensitive-action email code through a shared platform-core accounts-auth facade.
...
- Settle immediately in-platform:
  ...
  - Implemented: accepted-document evidence can generate PDFs/CSVs on demand from portal/admin history.
...
- Buyer and seller accepted-document artifacts can be generated on demand; legal terms/transaction-agreement PDFs are not emailed by default.
- Claim ownership changes atomically with ledger settlement.
- Seller proceeds become balance lots subject to the same 60-day holding rules.
...
- Implemented: seller self-cancel lifecycle tests.

### Phase 11: Risk, Collections, Recoveries, Public Notes, and Default Resolution
...
- Implemented foundation: recovery event:
  ...
  - implemented: universal payment-waterfall version stored as event metadata; priority is fixed to Garanta legal costs/recovery fee, penalty/default interest, contractual interest, then principal.
  - implemented: admin records outstanding penalty/default-interest and contractual-interest obligations; the server derives the applied category split and principal from current holdings. External/platform recovery costs are stored separately before lender-facing category allocation.
...
- Deferred: automatically calculate default/penalty interest from the official default declaration date. Current recovery event accepts the admin-declared outstanding default-interest obligation and stores universal-waterfall metadata.
- Implemented: apply the universal non-overridable waterfall and derive the actual split server-side. Admin-entered category allocations and project-specific priority overrides are not supported.
...
- Generate default/recovery report and affected-lender notification for each recovery payment.
- Implemented as an explicit admin-only final-resolution primitive: loss recognition requires a defaulted loan, written-off principal equal to remaining active holding principal, immutable per-investor loss lines, and holding closure. Garanta/legal/accounting still own the operational policy for when this endpoint may be used.
...
- Implemented: recovery allocations are deterministic, universally ordered, pro-rata by current principal balance for lender-facing buckets, rounded, and auditable.
- Default/recovery reports show gross-to-net recovery, recovery costs, Garanta recovery fee, waterfall category split, lender allocation, and rounding differences.
- Affected lenders receive recovery distribution notifications.
- Investors see loan status, days past due, and public notes.
- Implemented foundation: final default loss recognition closes remaining active holdings and records immutable per-investor loss lines. Advisor-approved report/PDF/CSV wording and the policy for when an admin may trigger final recognition remain required before production use.
...
- Implemented: defaulted-loan final loss-recognition tests cover status gating, exact principal conservation, holding closure, immutable per-investor loss lines, idempotency, API response, and append-only DB guards.
...
- Implement accounting exports:
  ...
  - implemented foundation: recovery/default-resolution rows.
...
- Implement operational reports:
  ...
  - implemented foundation: recovery/default-resolution report.
...
- Deposits:
  ...
  - payment reference.
  - reminders about the 60-day holding limit and loan-specific eligibility.
- Withdrawals:
  - bank details.
  - available balance.
...
- Implemented foundation: dashboard summary counters and short queues for:
  ...
  repayment-due loans, late/default/recovery risk loans, secondary-market
...
- Implemented foundation: reconciliation-break queue surfaces all snapshot
  ...
  breaks.
- Implemented foundation: admins can sync reconciliation-break snapshots into
  idempotent `payment_reconciliation` admin tasks through
  `/api/v1/admin-ops/reconciliation-break-tasks/sync/`. The sync skips clean
  snapshots, creates one task per anomalous snapshot, returns existing tasks on
  rerun instead of duplicating them, and records audit/domain evidence. The
  admin dashboard exposes a first-version `Create tasks` action on the
  reconciliation-breaks queue.
- Audit disposition: concurrent reconciliation-break task sync is addressed,
  not deferred. `payment_reconciliation` tasks for `ReconciliationSnapshot`
  records now have a database uniqueness backstop on snapshot identity, and the
  sync loop catches an `IntegrityError` from a competing sync and reuses the
...
11. The funding deadline passes with at least the published minimum subscription met.
12. The scheduled resolver closes funding at the accepted amount and creates holdings; the full accepted principal moves to the borrower disbursement payable and the planned fee is recorded as evidence only.
13. Admin finalizes the borrower disbursement; the borrower success fee is booked as Garanta revenue at disbursement.
14. The existing holding reflects the disbursed loan.
15. Investment document is available for on-demand generation in the investor/admin document history.
...
3. Day 30 passes; the lot can still fund a loan whose remaining subscription window fits within its original 60-day deadline. A longer window is rejected without changing the source balance.
...
8. Penalty-mode sources cannot be invested; FX and secondary-market purchases are blocked until overdue balance is resolved. Withdrawal remains available as the exit path.

### Scenario C: Repayment in Advance (Early Repayment)

1. Loan is healthy.
2. Next regular installment is 1,000 CHF.
3. Admin opens Loans > Manage > "Record borrower repayment"; the fixed regular amount does not match, so admin checks "Repayment in advance", enters 3,000 CHF, and selects the borrower repayment bank date.
4. System previews the allocation and both the old and the new schedule; admin confirms.
5. Payment covers all unpaid scheduled interest of installments due on or before the bank date.
6. Payment covers pro-rata accrued interest on the outstanding principal up to the bank date.
7. Remaining amount reduces outstanding principal.
8. Future installments are regenerated as a new schedule version with original due dates preserved and the first regenerated installment's interest prorated.
9. Investor distributions are credited to balances.
10. A full payoff would mark the loan repaid.
...
11. Accepted-document artifacts are available on demand; buyer/seller notification emails are sent where configured.
...
6. System applies the universal waterfall: legal costs/recovery fee, penalty/default interest, contractual interest, then principal, with contractual interest cutoff at default and separate default/penalty interest where applicable.
7. System allocates lender-facing recovery buckets pro rata to lender holdings based on current principal balance.
...
7. Full subscription auto-closes; at deadline, any positive round closes and an
   empty round cancels. Successful close atomically creates active holdings and
   post-boundary entitlements and moves escrow to originator payable. No boundary
   receipt or manual activation is required. The original boundary installment
   belongs wholly to the LO and is recorded separately when Garanta receives it.
   Processing failures preserve reservations and trigger urgent alerts plus
   automatic retry; admins cannot substitute cancellation for a qualified close.
...
- Sources remain potentially eligible before day 60; eligibility depends on the selected funding window.
- Primary-market orders require each reserved source's withdrawal-deadline date to be later than the final subscription date.
...
- Balances approaching 60-day holding deadlines.
...
- Legal approval for balances, FX, 60-day holding rule, forced withdrawal, and penalties.
...
- Every launch Loan Originator has completed offline KYB/AML, verified external
  settlement instructions and an operational owner. Platform company evidence is
  optional; current par subscriptions have no primary-market LO fee.
...
13. Build defaults, recoveries, default-resolution policy hooks, and public notes.
...
- Legal-entity lenders can be created by admin after Garanta completes company KYB/AML offline. They have no self-service Didit flow or required platform KYB document record; representatives must be active, unrestricted and phone-verified.
- Borrowers are legal-entity records only and have no portal. Garanta completes company KYB/AML offline; explicit compliance holds and adverse decisions still block transactions.
...
- Balance lots enforce 60-day holding limits with loan-specific funding eligibility.
- Balance-funded primary-market orders are blocked when consumed sources cannot cover the remaining funding window within their original holding deadlines.
...
- Performing Loan Originator claims can be imported, subscribed at par through a
  finite funding round, activated automatically and atomically at funding close,
  ...
  The boundary installment belongs entirely to the LO and is recorded separately
  when actually received, without delaying creation of the investment holding.
...
- **2026-09-03, amended 2026-09-07, PROD-DEC-010 through PROD-DEC-014 / MKT-DEC-022 through MKT-DEC-025:** current LO subscriptions use finite at-par funding and automatic activation at successful close. The immutable published schedule fixes post-boundary principal and interest/penalty rights; the boundary installment belongs entirely to the LO. No manual activation, new activation CSV or fictional bank receipt is needed. Actual repayments are separate append-only evidence. Company KYB files are optional because Garanta handles company compliance offline; explicit holds/adverse decisions remain blocking. Existing closed reservations are upgraded from original evidence, with immediate repair tasks/emails for exceptions. These rules supersede previous boundary-payment activation guidance, without reinterpreting historical legacy-yield purchases.
