...
- Deposits, repayment/installment proceeds, secondary-market proceeds, refunds credited to balance, and FX proceeds are subject to source-level balance ageing controls, including a 60-day holding deadline and loan-specific funding eligibility under the launch interpretation.
- Sources may be reserved only if their original 60-day holding deadline covers the loan's entire remaining subscription window. A 30-day window needs 30 days remaining; shorter windows allow older sources. At day 60, available balances enter the forced-withdrawal or missing-IBAN penalty/freeze process.
...
- Smart Invest is one investor-owned saved-filter discovery rule, not auto-investment. Its setup wizard contains collateral, currency, optional minimum yield, optional maximum term, and review only; it has no repayment/reinvestment preference or originator-concentration step. Newly published matches generate one transactional alert per investor and loan, while every investment still requires explicit review and confirmation.
- Every direct primary-market loan has an admin-set funding deadline and a disclosed minimum subscription threshold. The threshold defaults to 50%, may be overridden while the loan is a draft, and is immutable from publication onward. After the deadline, the platform automatically closes at the committed amount when the threshold is met or cancels and releases reservations when it is not. A failed automatic resolution is removed from the marketplace, preserves reservations, and opens an urgent operations case.
- The authoritative business timezone is Europe/Zurich for 60-day holding balance deadlines, reminders, penalties, funding deadlines, day-5 late status, day-16 default status, report day buckets, and scheduler cutoffs. Timestamps are stored in UTC.
...
- Published loan terms remain subject to the listing-edit policy, but the advertised financeable principal and disclosed minimum subscription are frozen at publication. Only deterministic deadline resolution may reduce principal to the subscribed amount when the frozen threshold is met, with required investor notification and a recorded reason.
...
- Every borrower payment uses one universal, non-overridable waterfall across direct and Loan-Originator loans and across active, late, and default/recovery handling: Garanta legal costs and recovery fee, then penalty/default interest, then contractual interest, then principal. A tier may be zero when no amount is due, but principal is never paid while a higher tier remains due. Admins and imported files supply evidence/obligations, not a caller-selected allocation; the server derives the split. Lender-facing distributable buckets are allocated pro rata to current lender holdings based on current principal balance. Recovery reports separately show gross recovered amount, third-party costs, recovery fee, net amount received by Garanta, fixed waterfall allocation, contractual-interest cutoff at default, default/penalty interest where applicable, lender distributions, and rounding differences.
