...
- Phase 12 reporting foundation is implemented as backend/API only. Admins can generate audited CSV report artifacts through `/api/v1/reporting/admin/reports/` for the first launch report definitions: `operational_subledger`, `trial_balance`, and `garanta_accrued_revenue`. The report request supports custom inclusive date ranges, `redacted` or `full` mode, optional filters such as currency, and a destination/handling note. Each generation creates immutable `ReportRun` and `ReportEvent` evidence with DB append-only triggers, report definition version, row count, manifest, CSV filename, content checksum, audit event, and domain event. CSV cells are neutralized against spreadsheet formula injection before output. Operational subledger exports one row per ledger posting with journal, account, source, amount, currency, tax metadata, and evidence fields; redacted mode masks investor IDs and bank/evidence references while full mode preserves them. Trial balance is cumulative as of `end_date`, groups debit/credit totals and signed balances by account/currency, and appends per-currency control-total rows so admins can verify total debits equal total credits. Garanta accrued revenue aggregates revenue account postings by currency, account type, and event type. The API returns CSV content in JSON for now so the source rows, checksum, manifest, and generated client can be tested before adding file-streaming. PDF rendering, ZIP evidence packages, Bexio-specific chart/import mapping, annual participant tax statements, broader operational reports, stored report artifact files/object-storage persistence, large-export streaming/range limits, report download UI, and admin-console report screens remain deferred to later Phase 12/admin-console/technical-hardening slices. Full exports intentionally remain admin-accessible under launch governance, default to redacted mode, and are audited; enhanced approval/reason-code policy is tracked as a Garanta reporting-governance follow-up.
...
- Implement report request system:
  - implemented foundation: admin-only on-demand report generation API.
  - implemented foundation: custom inclusive date ranges.
  - implemented foundation: CSV output returned with manifest/checksum metadata and formula-injection neutralization.
  - implemented foundation: immutable report-run and report-event evidence with audit/domain events.
  - monthly.
  - quarterly.
  - yearly.
  - PDF.
...
- Implemented foundation: redacted and full export variants for source-ledger reports.
- Implemented foundation: report definition versioning stored on each report run.
