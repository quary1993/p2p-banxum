...
- Phase 8 servicing repayment foundation is implemented as backend/API only for regular, partial, and healthy-loan early repayments. Admins can record a borrower repayment bank operation, default-match it to the next unpaid installment, require warning acknowledgement for underpayments and overpayments, apply the launch waterfall with fees/penalties inactive, allocate interest then principal, credit investor balances with new ageing balance lots, reduce active holding principal using exact `current_principal_minor` weights with deterministic residue handling, and queue idempotent `email.repayment_distribution_credited` notices for affected lenders. Early repayments on funded/current loans allocate the excess after current interest/principal to future principal, create a new immutable schedule version for the same remaining period with lower remaining principal, and preserve the current installment row so paid-total lookup remains schedule-version tolerant. Borrower repayment events and lender distribution lines are immutable, have application-level and DB-level append-only guards, and are exposed through `/api/v1/servicing/admin/borrower-repayments/`. Lender-payment-fee fields are reserved but operationally zero at launch; enabling non-zero lender payment fees requires a later ledger path that credits Garanta revenue instead of only investor liabilities. Admin evidence-file uploads, late/default scheduler, multi-installment catch-up payments for late loans, and extended recovery waterfall automation remain for later servicing/risk/communications slices.
