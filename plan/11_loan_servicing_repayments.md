...
If a supported borrower payment arrives after its contractual due date, the validated interest component is still conserved exactly: BANXUM does not invent additional interest. Entitlement allocation uses the actual payment value date, so the post-assignment holder's eligible accrual window extends through that date and the originator's pre-assignment share of the same fixed interest component may be correspondingly lower than the original quote projection. This investor-favorable timing effect is intentional, must be reflected in investor disclosures, and does not keep an opportunity open once the late/hold/default gates apply.
