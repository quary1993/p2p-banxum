...
- FX conversion does not reset balance ageing. The target FX-proceeds lot inherits the latest investment and withdrawal deadlines from the consumed source lots.
...
- Admin FX delta report by date range shows source sold, gross target bought, target credited, fees, and net external settlement amounts by currency.
...
- Admin can declare the actual external FX execution and see realized FX gain/loss without changing investor balances once the remaining external settlement slice is implemented.
...
- Remaining: external FX settlement declaration tests.
- Remaining: realized FX gain/loss calculation tests.
