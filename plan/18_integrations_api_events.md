...
Follow-ups:
Validate Yahoo Finance API/access terms, define polling interval, previous-day-average calculation, min/max pair bounds, exact quote expiry behavior in the UI, weekend/holiday quote policy, and alert recipients.

Implementation status:
Executable quote issuance now has a Yahoo Finance chart adapter behind `FX_RATE_PROVIDER=yahoo_finance`, with local/test-only mock fallback. The adapter records the provider name, quote id, timestamp, previous-day comparison value, and source payload hash reference, then relies on the FX service's freshness, bounds, and same-provider deviation checks before creating an executable quote. The real-provider integration audit is closed for current security posture: provider calls are fixed to configured Yahoo symbols for enabled pairs, bounded by timeout, and fail closed on provider/network/payload/sanity errors; deploy checks prevent non-local mock operation. Display-only background polling remains a separate deferred integration. Weekend/holiday executable FX remains an explicit launch-policy decision: the current default freshness window rejects stale Yahoo ticks rather than executing from an old market price.
...
Twilio Email API is the launch provider for transactional emails. The platform must store full sent email content, template version, data snapshot, recipient, provider operation id, and delivery/failure metadata where available. Failed sends retry a configured number of times and then create an admin notice/task. Future marketing/newsletter audience management is a separate integration decision. The private-test sender domain is `nxnarena.com`; the final production sender domain remains a go-live decision.

Twilio Verify is used for phone verification. Twilio Email API handles security and operational email; the credentials and provider paths remain separate.

Implementation status:
Twilio Email API dispatch, legacy SendGrid rollback support and Twilio Verify phone verification are implemented behind provider settings and non-local deploy checks. Business-event-to-email outbox mapping exists for balance-ageing reminders, repayment credits, recovery distributions, and secondary-market listing/purchase events. Provider DNS/domain validation, bounce/suppression handling, Twilio live delivery testing, and final legal/template wording remain launch setup tasks.
...
1. Answered by COMMS-DEC-001/002: selected launch communication vendor is Twilio Email API for transactional email and Twilio Verify for phone verification, in addition to Didit for KYC/AML. Marketing contact-list management remains future scope.
