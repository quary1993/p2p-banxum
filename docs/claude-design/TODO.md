...
- Current behavior: the investor frontend now self-hosts the `Instrument Sans` and `Newsreader` variable fonts and uses the BANXUM redesign palette (`website_redesign/styles.css`) through the production token layer. The existing sidebar navigation, routes, generated API hooks, balance projection, loan-detail route, investment modal, and all backend write flows are unchanged. The Marketplace page now has a compact editorial introduction, current investable-balance band by currency, factual primary-order lifecycle explainer, Focused/Detailed modes, search across title/purpose/collateral/reference/rating, currency and Open/All filters, four real sort modes, responsive opportunity rows, and a detailed order-mechanics modal. Every field from the previous marketplace table remains visible: title, loan reference, purpose, amount, currency, target interest, term, rating, funding percentage, status, and refinancing provenance; remaining capacity, committed amount, collateral/backing, funding deadline, and first-come-first-served context are also surfaced from the existing projection.
- Design decision: `website_redesign/` is a visual/IA reference, not executable production code. Its future auto-invest/rule card and fabricated 30/60-day future-capacity figures were deliberately not ported because auto-invest is out of v1 scope and the current investor API does not expose forecast liquidity. The implemented capacity band uses only server-projected `investable_minor` and explicitly warns that loan-specific balance-lot deadline checks still apply. The page keeps operational density and does not reproduce the reference's desktop-only minimum width or static demo numbers.
- Remaining backend/API dependency: the marketplace preview contract intentionally remains limited and does not currently expose borrower legal name/country, repayment type, LTV, collateral value, repayment history, originator stake, or installment-payment status. These remain available where applicable on the authenticated full loan-detail projection. Add them to the backend preview/list projection only after product/compliance approves showing them across all marketplace rows; do not derive them client-side or copy disclosure-visibility logic into the frontend.
