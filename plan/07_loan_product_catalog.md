...
- Draft and published funding periods contain at most 50 inclusive Europe/Zurich subscription dates.
- The default direct-loan funding period is 30 inclusive dates (today through today + 29). Publication rejects past deadlines and freezes the deadline. Sources are eligible only if they cover the remaining funding window within their original 60-day holding limit.

Rationale:
These defaults make schedule generation, arrears status, investor display, and funding-campaign monitoring deterministic for v1 while preserving admin planning flexibility inside a controlled range. Deriving the first installment from the loan start date removes a redundant input and keeps schedules deterministic. Refinancing loans keep servicing identical to standard loans: the platform funds and services only the financeable principal, while the declared original loan data and its computed schedule stay informational for investors, who see a "Refinanced loan" badge and the original schedule on the loan detail page. The publish-time paid ticks and remaining-outstanding check prevent investors from financing more than what is actually outstanding on the original loan. The 50-date campaign cap and per-source remaining-window check work together: each reserved source must cover the final subscription date within its original holding limit.
...
- Every new direct or LO v2 opportunity requires a finite funding deadline within 50 inclusive subscription dates; direct loans default to 30 dates. Source eligibility is checked against the actual remaining window, not a fixed source-age cutoff.
...
A Loan Originator is an admin-managed counterparty with no portal login or investor wallet. Its internal payable tracks money Garanta must transfer externally. Registration, contact/settlement details, risk notes and active/blocked status are retained; company compliance is handled offline and a platform KYB evidence reference is optional. The 50% premium-sharing fee applies only to historical `legacy_yield_v1` contracts. Current `par_component_v2` subscriptions have no primary purchase fee.

### PROD-DEC-012: Borrower Snapshot and Imported Contract

An originator claim has no `BorrowerEntity` link. It stores a loan-specific final-borrower snapshot. Legal name is private by default and is disclosed only when an admin explicitly enables publication for that loan. Otherwise investors and agreement copies use the public display name. Ownership, bank, KYB/AML and financial-risk notes remain internal. Historical acceptance evidence is immutable; copies redact an internal name that has no recorded publication permission.
...
Every `par_component_v2` opportunity has an admin-defined funding window of at most 50 inclusive subscription dates. Existing published rounds retain their disclosed zero minimum: a positive subscription closes at deadline, while an empty round cancels. Reaching sellable capacity closes automatically. The per-investor/order minimum remains loan-specific. Applying a configurable default minimum to new LO rounds awaits explicit product confirmation; it must not silently rewrite published agreements.

Amended 2026-09-07: closing a round atomically closes unallocated intents, creates active par holdings and post-boundary entitlements from the published immutable schedule, hides the opportunity, and transfers funding escrow to the internal Loan Originator payable. There is no awaiting-activation stage and no manual activation import. The LO has no platform login or wallet; the payable records Garanta's external settlement obligation.

Before the boundary payment, total borrower principal includes the boundary principal, while investor holdings own only their purchased post-boundary principal. The difference remains LO-owned. Garanta records the boundary installment through the ordinary LO borrower-repayment workflow: its principal, interest and penalty belong entirely to the LO, while evidenced Garanta legal/recovery costs retain their first-tier priority. Investor holdings are unchanged by that installment. Subsequent eligible payments use current ownership and the frozen participation percentages. Regular-payment imports cannot change the agreed schedule; actual prepayments use the prepayment workflow and retain historical evidence.

Existing closed-but-unactivated reservations are upgraded transactionally from their original close and schedule evidence, without fabricating a bank payment or modifying old audit records. Explicit holds or inconsistent historical data remain operational exceptions requiring reviewed resolution; they do not authorize a new manual activation/repricing flow.

V1 deliberately does not resize an already-open round after a borrower payment or prepayment. Such a change alters the offered asset and requires full cancellation/refund plus a replacement opportunity. A future partial-trim flow requires an approved deterministic policy for investor allocation, minimum-order treatment, minor-unit rounding, disclosure, and consent before implementation.

If close processing fails for an operational reason, the whole close transaction rolls back, allocated funds remain reserved, the opportunity becomes non-public, and an urgent admin task plus an email to the configured operations address are created. The scheduler retries the published outcome; admins repair the cause but cannot cancel a subscribed failed or expired round instead. Routine Loan Originator KYB expiry does not block close; an explicit adverse status or compliance hold does. A repayment, prepayment, or economic schedule change discovered while funding is still open requires cancellation/refund and replacement publication before the deadline. Changes discovered after the deadline are urgent repair exceptions, not permission to rewrite accepted terms. After successful close, holdings and borrower payments follow servicing. Resolution cannot use a future admin-supplied date; staging time travel must use the QA clock.
