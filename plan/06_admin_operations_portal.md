...
The combined admin Loans table separates "Loan interest rate" from "Investor yield", showing both as annual percentages to two decimal places. Loan interest is the contractual borrower rate. Investor yield uses the loan rate for direct loans and the originator profile's investor rate for LO loans. For current LO subscriptions this is the nominal coupon multiplied by the investor's interest participation, not a realized return; legacy claims retain their effective annual target yield. These are display projections, not editable rate overrides.
