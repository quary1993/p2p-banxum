...
The investor dashboard may present several Smart Invest matches for explicit selection and place up to 20 selected direct-loan orders and Loan Originator claim purchases as one batch. This is not auto-invest: the investor explicitly selects every opportunity, reviews the exact cash amount for every investment, accepts the current primary-market terms for that list, enters one fresh sensitive-action email code, and confirms the batch.

The reviewed allocation may contain both CHF and EUR opportunities, while every balance movement remains strictly isolated in its own currency. Before the terms-and-code step, the client requests one independent executable five-minute quote for every selected Loan Originator claim. The reviewed item snapshot binds each claim's quote ID and exact cash consideration; it also shows the assigned claim principal because cash price and assigned principal may differ. An expired or changed quote must be refreshed and reviewed again before confirmation.

The backend locks direct loans in stable ID order, then validates and locks each quoted Loan Originator claim through its normal quote/purchase primitive. Direct items must satisfy the launch minimum and reserve their exact reviewed amount from same-currency balance. Loan Originator items must settle the exact quoted cash amount, respect the claim-specific minimum, latest schedule revision, remaining sellable principal, skin-in-the-game floor, and quote expiry. Direct items become allocated orders awaiting funding close; Loan Originator items complete their assignment and holding purchase immediately. One outer database transaction rolls back every child order, reservation, claim purchase, holding, ledger posting, derived acceptance, and counter across every currency if any selected item fails. The append-only umbrella acceptance remains as internal evidence of the investor's reviewed placement attempt.

An immutable parent batch record binds the investor, per-currency cash totals, item snapshot, direct-order IDs, Loan Originator purchase IDs, umbrella acceptance, idempotency key, and request fingerprint. Legacy single-currency batches retain their scalar currency/total fields; mixed-currency batches never store or display a meaningless cross-currency scalar total. Exact replays return the parent record without consuming a second code; conflicting replays are rejected. The umbrella clickwrap snapshot must exactly match the submitted loans, amounts, quote IDs, and server-owned loan currencies. The system derives current server-owned child evidence with `primary_order` context for every direct order and `originator_claim_quote` context for every quoted claim, so each investment remains independently reproducible and downloadable; the internal umbrella authorization is not exposed as a duplicate investor document.
