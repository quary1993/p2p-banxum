...
- Phase 8 servicing repayment foundation is implemented as backend/API plus the Loans-table Manage dialog UI for regular installments and repayments in advance. Servicing and recovery actions live in the Loans table Manage dialog; "Record borrower repayment" shows the loan's current schedule. A regular installment declaration uses a fixed amount that must equal the outstanding amount of the next due installment and cannot be edited; the earlier warning-acknowledged partial/overpayment mechanism was removed in the refinancing-loans slice. Any other amount (except defaulted loans, which go through recovery) is declared as a "Repayment in advance" with a "Borrower repayment bank date". The advance waterfall pays all unpaid scheduled interest of installments due on or before the bank date, plus pro-rata accrued interest on the outstanding principal for the days since the last installment due date up to the bank date (outstanding x monthly rate x days elapsed / days in period, monthly rate = annual bps / 120000), and reduces principal with the remainder; the future schedule is regenerated as a new immutable schedule version containing only future rows with original due dates preserved and the first regenerated installment's interest prorated for the remainder of the current period. Before anything is written, the admin confirms a preview showing the allocation and both the old and the new schedule (`POST /api/v1/servicing/admin/borrower-repayments/advance-preview/`); a full payoff through repayment in advance marks the loan repaid. The launch waterfall runs with fees/penalties inactive, credits investor balances with new ageing balance lots, reduces active holding principal using exact `current_principal_minor` weights with deterministic residue handling, and queues idempotent `email.repayment_distribution_credited` notices for affected lenders. Borrower repayment events and lender distribution lines are immutable, have application-level and DB-level append-only guards, and are exposed through `/api/v1/servicing/admin/borrower-repayments/`. Lender-payment-fee fields are reserved but operationally zero at launch; enabling non-zero lender payment fees requires a later ledger path that credits Garanta revenue instead of only investor liabilities. Multi-installment catch-up amounts for late loans are handled through the repayment-in-advance flow; admin evidence-file uploads and extended recovery waterfall automation remain for later servicing/risk/communications slices.
- Phase 8 servicing status scanning is implemented through backend services, scheduled jobs, and admin servicing controls. Admins can run `/api/v1/servicing/admin/status-scan/` for all active funded/late loans or a selected loan list; the scanner uses Europe/Zurich calendar-day due-date deltas, marks loans late on day 5 and defaulted on day 16 based on the first unpaid installment, marks loans repaid when no scheduled installment remains outstanding, records audit/domain/loan events for every status transition, and leaves already-defaulted loans to the recovery workflow. Normal borrower repayment is allowed for funded and late loans, blocked for defaulted loans, and a catch-up repayment that clears the overdue installment automatically moves a late loan back to funded/current or repaid when all installments are cleared. The platform-core scheduled-job runner can execute the status scan daily with `business_date(now_utc())` semantics. Admin task creation from status changes, investor/borrower notifications, and default recovery processing remain deferred. Status-scan batching/per-loan transactions remain a scale hardening item; the current scan is acceptable for launch-scale portfolios.
...
- Phase 10 secondary-market listing and purchase/settlement are implemented through backend services, investor marketplace screens, and admin approval operations. Investors with financial access can list one full active holding, not a split or partial holding, through `/api/v1/marketplace/secondary/listings/` after secondary-market listing clickwrap evidence exists for that holding, that evidence still references the current published listing-terms template version, and a fresh `secondary_market_listing` sensitive-action email code is consumed through the shared platform-core facade. Performing/funded loan holdings publish automatically; late/defaulted holdings create an approval-requested listing that is not visible until an admin approves it with reason and public disclosure note. The listing snapshot stores current principal, price bps, transfer price, discount/premium, ACT/365 accrued interest through the listing date for performing loans, maker/taker bps, configurable per-currency minimum fees, seller net proceeds, buyer total cost, loan status, days past due, last payment date, and whether a buyer risk acknowledgement is required. Listing creation, seller self-cancel, and admin approve/reject/remove transitions are idempotency-fingerprinted, audit logged, domain-event recorded, backed by append-only secondary listing events with DB-level immutability triggers, and queue seller-facing `email.secondary_market_listing_status` notices. Active-listing visibility is financial-access gated, hides listings whose current loan status no longer matches the status disclosed at listing/approval time, and uses a buyer-facing serializer that omits seller identity, seller proceeds, maker-fee economics, document evidence IDs, and admin audit fields. Sellers can cancel their own active or approval-requested listing through `/api/v1/marketplace/secondary/listings/{listing_id}/cancel/`; cancellation records seller ID, reason, timestamp, audit/domain evidence, and frees the holding for a later relisting without touching holdings or ledger balances. Buyers can purchase an active listing through `/api/v1/marketplace/secondary/listings/{listing_id}/purchase/` after secondary-market purchase clickwrap evidence exists for that listing, that evidence still references the current published purchase-terms template version, and a fresh `secondary_market_purchase` sensitive-action email code is consumed through the shared platform-core facade; approved non-standard listings additionally require buyer risk acknowledgement. Purchase settlement recomputes accrued interest, buyer total, seller net, and maker/taker fees as of the actual purchase date, rejects stale loan-status disclosure, consumes eligible buyer balance lots through the ledger, posts a balanced buyer-liability debit / seller-liability credit / Garanta platform-fee revenue journal, credits the seller net proceeds as a new secondary-market-proceeds balance lot, transfers the full holding to the buyer, marks the seller holding transferred, marks the listing sold, stores immutable purchase evidence with DB-level append-only triggers, and queues buyer/seller `email.secondary_market_purchase_confirmation` notices without exposing counterparty identity. Assignment/transfer document PDF generation and final legal notification wording/templates remain document and legal follow-up items.
...
- Implement borrower disbursement workflow and evidence upload:
  - Backend ledger/API finalization is implemented with the default funded-principal-minus-fee amount, joint amount/fee override with required note, transfer + fee <= funded validation, and remainder retained in the borrower payable.
  - The disbursement action is available from the Loans table Manage dialog for funded loans and from the Finance ops card; uploaded bank-statement/evidence-file attachments remain to be built.
...
- Apply the repayment-in-advance waterfall:
  ...
  - pro-rata accrued interest on the outstanding principal from the last installment due date to the bank date (outstanding x monthly rate x days elapsed / days in period, monthly rate = annual bps / 120000).
...
- Regenerate the future schedule as a new version after a repayment in advance: only future rows, original due dates preserved, first regenerated installment interest prorated; mark the loan repaid on full payoff.
