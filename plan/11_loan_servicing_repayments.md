...
### SERV-DEC-015: Post-Boundary Component Entitlements

Garanta services a current originator subscription only after activation. The borrower pays Garanta. Each payment first follows the same universal waterfall as a direct loan: Garanta legal costs/recovery fee, penalty, contractual interest, then principal. The activation-boundary installment belongs entirely to the Loan Originator; investor entitlement begins only with schedule rows strictly after that date.

For later payments, principal is allocated by current investor-owned versus originator-owned principal. The investor pool's attributable interest and penalty are each multiplied by the separate loan-level participation bps declared before publication; the originator receives the remainder. Largest-remainder splits reconcile every investor component exactly. Investor amounts create balance lots and originator amounts create servicing payable. Secondary transfers move the holding's principal and all associated future component rights together.

Historical `legacy_yield_v1` holdings continue to use their immutable dated assignment/accrual evidence; they are not reinterpreted as current subscriptions.
...
For penalties, "recomputes" means BANXUM derives the cumulative amount due from the immutable imported schedule less prior validated payments. BANXUM does not independently derive an originator loan's contractual penalty amount from a separately stored penalty-rate policy in v1. The imported schedule remains evidence rather than unchecked authority because amount conservation and the universal waterfall are enforced, but originator onboarding and import controls must validate that its penalty schedule matches the underlying contract. Independent penalty-policy storage and recomputation are a future hardening option, not launch behavior.

### SERV-DEC-016: Payments, Holds, and Subscription Integrity

Regular and advance repayments reuse loan-type-specific schedule calculations but always use the universal payment-allocation priority. The imported CSV component split is evidence, not authority, and is rejected if it attempts to pay a lower tier before a higher due tier. After activation, each payment atomically updates schedule state, holdings, originator-owned principal, entitlement projections, and secondary listings; listings retain their discount/par percentage while repricing current principal. Current originator holdings cannot be listed at a premium. No restructuring exists: extensions settle the old loan and create a new one. Garanta handles recovery without originator recourse/buyback.

Amended 2026-09-07: activation is part of successful funding close, not a later admin action. A borrower payment, prepayment or material schedule change while funding is still open requires cancellation and a replacement opportunity; investor rights must not be silently repriced. An explicit hold blocks close and preserves reservations for reviewed resolution. After close, record the real boundary installment through servicing and allocate all its components to the LO; holdings already exist and subsequent investor entitlements remain tied to the published schedule. Company KYB is offline: optional record absence/expiry does not block transactions, but explicit adverse/review decisions and compliance holds do.
