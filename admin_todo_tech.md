...
- The password hash is generated with `python backend/manage.py generate_password_hash --stdin`
  or `python backend/manage.py generate_password_hash <password>` and stored as an
  environment secret, not committed.
- The environment-managed superadmin is synchronized with `make bootstrap-superadmin`
  after the env values are present.
- Rotation:
  - generate a new password hash.
  - update the production env file/secret source.
  - redeploy/restart the app.
  - run `make bootstrap-superadmin`.
  - verify superadmin login.
- Removal:
  - set `GARANTA_SUPERADMIN_ENABLED=false` or remove the env values and redeploy.
  - run `make bootstrap-superadmin` so env-managed superadmin accounts are locked.
