...
- Primary-market funding cancellation foundation is implemented. Admins can cancel a published campaign through `/api/v1/marketplace/primary/admin/loans/{loan_id}/cancel-funding/` when the loan has not been closed/funded, or run an admin expiry scan through `/api/v1/marketplace/primary/admin/loans/expiry-scan/` for published campaigns whose funding deadline has passed. Cancellation releases every allocated order through the same ledger/order release primitive used for manual pre-close releases, restores the exact balance-lot allocations, posts reversing escrow-to-investor-liability journals, decrements committed principal to zero, closes pending intents as `closed_not_invested`, moves the loan to `cancelled`, and records immutable `PrimaryLoanCancellation` evidence plus audit/domain/loan events. An investor message is required when any pending or allocated orders exist, so the cancellation has a documented investor-facing explanation even before notification delivery is wired. The admin console exposes manual cancellation and expiry scan actions with confirmation dialogs. The platform-core scheduled-job runner can execute campaign-expiry scans daily. Cancellation/expiry notification emails and generated cancellation evidence PDFs remain deferred to later primary-market/communications/documents slices.
...
- For accepted partial funding, set the accepted funded amount as final financeable principal, regenerate the repayment schedule from that principal with the same term, interest percentage, and percentage BANXUM fee, and require borrower repayment of that principal plus agreed interest. Refinancing original loan data is preserved.
