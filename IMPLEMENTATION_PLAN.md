...
- `borrower_can_transact` is the shared borrower transaction gate for loan publication, borrower disbursement, borrower repayment processing, and later borrower transaction mutations. Deterministic funding close uses a narrower close-specific gate: routine KYB expiry after publication is accepted, while compliance holds and adverse/review statuses remain blocking and enter the preserved-reservation failure workflow.
...
- Enforce funding deadline defaults:
  ...
  - publication rejects deadlines in the past or more than 29 days out, so a balance source pledged near the end of its 30-day investment window can still settle inside the 60-day operating limit.
...
- Implement primary-market balance-source eligibility at pledge/allocation time:
  ...
  - loan funding deadline does not need to be on or before each consumed source lot's 30-day investment/reinvestment deadline.
...
- If resolution fails, unpublish into `funding_close_failed`, preserve reservations, create an urgent admin task, email operations, and allow only deterministic retry or cancellation/refund.
...
12. Admin closes funding; the full accepted principal moves to the borrower disbursement payable and the planned fee is recorded as evidence only.
...
- Primary-market orders can consume source lots while those lots are still inside the 30-day investment window, even if the loan funding deadline is later than the source lot's day-30 investment deadline.
