...
- Write-off is not an operational v1 concept. Loans should remain `defaulted` while Garanta handles recovery/resolution. The earlier backend write-off evidence foundation must not be treated as launch functionality or exposed as a normal admin action unless Garanta later reverses this decision with legal/accounting approval. Future recovery-closure/loss-recognition work should resolve defaulted loans without introducing an automatic write-off state.
