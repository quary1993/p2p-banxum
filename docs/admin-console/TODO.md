...
- Current first-version behavior: the Loans panel exposes manual pre-deadline cancellation and deterministic funding-deadline resolution through generated API hooks. Manual cancellation requires a reason, includes an investor message field, and uses a danger confirmation dialog that explains balance-reservation release and immutable evidence. For expired loans, the resolver uses the threshold frozen at publication: at/above it closes at the subscribed amount; below it cancels and refunds. A failed resolution becomes non-public, preserves reservations, and appears as an urgent operations task for targeted retry or cancellation. Selected loan context shows status, funding deadline, committed principal, and total principal.
...
## 2026-08-05: Deterministic Funding-Deadline Resolution

- Screen or component: Loans table Manage dialog, funding queues, and urgent task handling.
- Current first-version behavior: direct-loan drafts default to a 50% minimum subscription and allow a case-by-case override before publication. Publication freezes that disclosed threshold. Once the Europe/Zurich deadline passes, the same resolver used by the scheduler and admin compares committed principal with the exact rounded-up threshold under the loan row lock. It closes full/partial automatically when met and cancels/refunds below it; there is no discretionary close button. Routine KYB expiry does not block close, while explicit compliance holds and adverse/review states remain blocking controls. Processing failures remove the opportunity from investor browse, preserve all reservations, create/reopen one urgent task, and send the configured operations email.
- Required admin-console improvement: add a structured failure investigation view that surfaces the failure event, committed/order conservation summary, task timeline, retry result, and cancellation/refund evidence without asking operators to interpret raw metadata.
- Priority: important.
...
- Current first-version behavior: the create/edit forms expose a "Refinancing loan" checkbox that reveals original principal/interest/term/repayment-type/interest-only/start-date inputs and renders a readonly client-side preview of the original schedule using the selected original repayment type. The first-payment-date input was removed; the backend derives it as loan start date + 1 month. Payloads only carry original fields when the checkbox is on. The Loans table shows each loan's canonical title together with its borrower legal name, UUID, status, and funding progress so operators do not have to infer the borrower from an identifier.
